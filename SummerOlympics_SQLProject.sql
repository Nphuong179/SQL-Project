...
with year_table as (
    ...
    from (
		select distinct year from olympics order by year asc) as years),
...
from(
	select country, sport, year, row_n, lag(row_n) over(partition by country, sport order by year) as pre_row_n
...
summary_country_sport_gold_momentum as(
    select country, sport, count(*) as count_n
...
select s.sport, country, count_n
from summary_country_sport_gold_momentum s
join (
	select sport, max(count_n) as max_count
